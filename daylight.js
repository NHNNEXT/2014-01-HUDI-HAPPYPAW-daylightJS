...
// 2. Framework을 만들때 신경쓸만한 것.
// 2.1 사용자가 편리하게 사용할지부터 생각하자.(내부코드의 아름다움보다는 사용자 관점에서 API 설계가 되야 한다.)
// 2.2 프레임워크가 chaining 을 지원하는 건 꽤 중요함.
// 2.3 성능비교보다는 일관된 형태의 API를 지원하는 게 중요함 
// 	   예를들어
// 	   argument의 순서가 일관되어 있다거나. 반환값의 형태가 일관된다던가 하는 걸 잘 지켜줘야 사용자는 이 API가 어떻게 동작되는지 쉽게 기억할 수 있음
// 2.4 프레임워크 소스를 그대로 퍼오는 것보다 착안해서 구현하는 게 좋겠음. (학습차원에서) 꼭 가져와야 한다면 어디서 가져왔는지 주석으로 정확히 명시하는 게 좋음.

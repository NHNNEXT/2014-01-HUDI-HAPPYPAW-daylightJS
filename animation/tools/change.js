	...
	tools.setProperty = function(pos, time) {
		...
		if(figure) {
			for(var i = 0; i < browserPrefixLength; ++i) {
				if(cssObject.hasOwnProperty(browserPrefix[i] + "transform"))
					delete cssObject[browserPrefix[i] + "transform"];
			}
			if(cssObject.hasOwnProperty("opacity"))
				delete cssObject["opacity"];
				
			if(cssObject.hasOwnProperty("left"))
				cssObject.left = parseFloat(dlElement.css("left")) + parseFloat(dlElement.css("border-left-width"));
			if(cssObject.hasOwnProperty("top"))
				cssObject.top = parseFloat(dlElement.css("top")) + parseFloat(dlElement.css("border-top-width"));
